...
def detect_face(model, cascade_filepath, image):
    ...
    # 얼굴인식 실행
    cv2.CascadeClassifier(cascade_filepath)
    faces = cascade.detecMultiScale(image_gs, scaleFactor=1.1, minNeighbors=2, minSize=(64,64))

    # 얼굴이 1개 이상 검출된 경우
    if len(faces) > 0:
        print("인식 된 얼굴의 수:{len(faces)}")
        for (xpos, ypos, width, height) in faces : #xpos, ypos, width, height
            ...
            #인식한 얼굴의 사이즈 축소
            face_image = cv2.resize(face_image, (64,64)) #머신러닝하기 위한 정보전달하기 위한 사진 
...
def main():
    ...
    if image is None:
        print("이미지 파일을 읽을 수 없습니다.{image_file_path")
        return RETURN_FAILURE
